...
public class RaterController {
	...
	public String registerPOST(RaterVO vo) throws Exception {
	...
	public void readPage(@RequestParam("raterId") String raterId, @ModelAttribute("cri") SearchCriteria cri, Model model)
			throws Exception {
		logger.info("readPage.....");
		model.addAttribute(service.read(raterId));
	}
	...
	public String modifyPageGET(@RequestParam("raterId") String raterId, HttpSession session,
		...
		// 2-1) 게시글 정보 가져오기
		RaterVO product = service.read(raterId);
		...
		if (rater.getRaterId().equals(product.getRaterId())) {
			...
			return "redirect:/rater/readPage";
		}
	}
